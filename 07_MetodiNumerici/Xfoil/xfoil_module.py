...
#                startupinfo=None, creationflags=0):
    ...
    ps = sp.Popen(['/home/claudio/bin/xfoil'],
                  stdin=sp.PIPE,
                  stdout=sout,
                  stderr=None,
                  startupinfo = None)
...
def find_coefficients(airfoil, alpha, Reynolds=0, iteration=10, NACA=True, delete=False, PANE=False):
    ...
    if not os.path.isfile(filename):
        call(airfoil, alpha, Reynolds=Reynolds, output='Polar', iteration= iteration, NACA=NACA, PANE=PANE)
    coefficients = {}
    # Data from file
...
def find_pressure_coefficients(airfoil, alpha, Reynolds=0, iteration=10,
    ...
    if not use_previous:
        ...
        if not os.path.isfile(filename):
            call(airfoil, alpha, Reynolds=Reynolds, output='Cp', iteration=iteration, NACA=NACA, PANE=PANE)
    coefficients = {}
    # Data from file
